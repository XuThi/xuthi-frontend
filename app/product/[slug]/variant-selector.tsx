...
// Built-in Vietnamese translations for common option attribute labels.
// Used as fallback when optionNames from the API doesn't provide a translation.
...
function processVariants(variants: Variant[]): VariantGroup[] {
    ...
    const hasCombinations = variants.some(
        (v) => v.combinations && v.combinations.length > 0,
    )
            ...
            if (hasAttributes) {
                ...
                return variants.find((v) =>
                    v.combinations?.every(
                        (c) =>
                            options[c.variantValue.variantType.label] ===
    ...
    const isOptionAvailable = (groupLabel: string, optionValue: string) => {
        ...
        if (hasAttributes) {
            return variants.some((variant) => {
                if (!variant.attributes || variant.stockQuantity <= 0)
                    return false
        ...
        return variants.some((variant) => {
            if (!variant.combinations || variant.stockQuantity <= 0)
                return false
            const combinationMap: Record<string, string> = {}
            for (const c of variant.combinations) {
                combinationMap[c.variantValue.variantType.label] =
                    c.variantValue.value
            }
    ...
    return (
        <div className="space-y-8">
            {variantGroups.map((group) => {
                ...
                const normalizedGroupLabel = normalizeKey(group.label)
                const displayLabel =
                    optionNames?.[group.label] ??
                    LABEL_TRANSLATIONS[normalizedGroupLabel] ??
                    group.label
